...
private:
    ...
    void split(Node *now, int val, Node *&left, Node *&right)
    {
        if (now == pool)
        ...
        {
            left = now;
            split(now->rchild, val, now->rchild, right);
        }
        else
        {
            right = now;
            split(now->lchild, val, left, now->lchild);
        }
    }
...
public:
    ...
    int query(int value)
    {
        ...
        for (Node *pos = r; pos != pool; pos = pos->lchild)
            rv = pos->value - value;
        int res = std::min(lv, rv);
        root = merge(res != Inf ? merge(l, ins) : l, r);
        return res;
    }
};
